...
def _insert_fact(connection: sqlite3.Connection, fact_id: str, group_id: str, value: str, recorded_at: str) -> None:
...
@pytest.mark.parametrize("group_id", ['["atlas","ship"]', '["atlas",null]', "[null,null]"])
...
def test_the_migration_retires_all_but_the_newest_active_value(chronicle_sandbox, loaded_manifest, tmp_path) -> None:
    ...
    with open_connection(replace(config, migrations_dir=early)) as connection:
